...
from ..result import AcademicMetrics
from ..weights import (
    INFLUENCE_WEIGHTS,
    VENUE_SCORES,
    get_citation_score,
...
class AcademicMetricsCalculator:
    ...
    def compute(self, paper) -> AcademicMetrics:
        """
        计算论文的学术影响力指标
        ...
        metrics.citation_score = get_citation_score(citation_count)
    ...
    def compute_score(self, paper) -> float:
        """
        计算论文的学术影响力总分
    ...
    def explain(self, paper) -> str:
        ...
        # 发表渠道解释
        venue = getattr(paper, 'venue', None) or "未知"
        if metrics.venue_tier == 1:
            parts.append(f"发表于顶会: {venue}")
        elif metrics.venue_tier == 2:
            parts.append(f"发表于优秀会议: {venue}")
        else:
