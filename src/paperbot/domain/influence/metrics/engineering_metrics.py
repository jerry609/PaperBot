...
from ..result import EngineeringMetrics
from ..weights import (
    INFLUENCE_WEIGHTS,
...
class EngineeringMetricsCalculator:
    ...
    def compute(
        ...
        has_code_attr = getattr(paper, 'has_code', False)
        has_code = bool(github_url or has_code_attr or code_meta)
        ...
        if code_meta:
            # GitHub Stars 评分
            stars = getattr(code_meta, 'stars', 0) or 0
            metrics.repo_stars = stars
            metrics.stars_score = get_stars_score(stars)
            
            # 可复现性评分（如果有）
            repro_score = getattr(code_meta, 'reproducibility_score', None)
    ...
    def _estimate_reproducibility(self, code_meta) -> float:
        ...
        # 有 README
        if getattr(code_meta, 'has_readme', False):
            score += 30
    ...
    def _is_recently_updated(
        ...
        Returns:
            是否最近更新
        """
        updated_at = getattr(code_meta, 'updated_at', None)
        last_commit_date = getattr(code_meta, 'last_commit_date', None)
        
        if not updated_at and not last_commit_date:
            return False
        
        try:
            last_update_str = updated_at or last_commit_date
    ...
    def _compute_activity_score(self, code_meta) -> float:
        """
        计算活跃度评分
        ...
        # Forks 数
        forks = getattr(code_meta, 'forks', 0) or 0
        if forks >= 100:
            score += 30
        elif forks >= 10:
            score += 20
        ...
        open_issues = getattr(code_meta, 'open_issues', 0) or 0
        if 1 <= open_issues <= 50:
            score += 20
        elif open_issues > 50:
            score += 10
