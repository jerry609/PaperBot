...
# 别名，兼容旧代码
RecommendationLevel = InfluenceLevel
...
class InfluenceResult:
    ...
    # 推荐级别
    recommendation: InfluenceLevel = InfluenceLevel.OPTIONAL
    ...
    def __post_init__(self):
        ...
        if self.total_score >= 80:
            self.recommendation = InfluenceLevel.HIGHLY_RECOMMENDED
        elif self.total_score >= 60:
            self.recommendation = InfluenceLevel.RECOMMENDED
        elif self.total_score >= 40:
            self.recommendation = InfluenceLevel.OPTIONAL
        else:
            self.recommendation = InfluenceLevel.LOW_PRIORITY
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "InfluenceResult":
        """从字典创建"""
        # 解析推荐级别
        recommendation = InfluenceLevel.OPTIONAL
        rec_value = data.get("recommendation", "")
        for level in InfluenceLevel:
