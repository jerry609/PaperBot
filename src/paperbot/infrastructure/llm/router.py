    ...
    def from_env(cls) -> "ModelRouter":
        ...
        if default_model_override:
            default_config = ModelConfig(
                ...
                model=default_model_override,
                cost_tier=1,
                api_key_env=os.getenv("LLM_DEFAULT_API_KEY_ENV", "OPENAI_API_KEY"),
                base_url=os.getenv("LLM_DEFAULT_BASE_URL") or None,
            )
        elif os.getenv("NVIDIA_MINIMAX_API_KEY") and os.getenv("NVIDIA_MINIMAX_MODEL"):
            default_config = ModelConfig(
                provider="openai",
                model=os.getenv("NVIDIA_MINIMAX_MODEL", "minimaxai/minimax-m2.1"),
                cost_tier=1,
        ...
        if reasoning_model_override:
            reasoning_provider = os.getenv("LLM_REASONING_PROVIDER")
            if not reasoning_provider:
                reasoning_provider = (
                    "anthropic" if "claude" in reasoning_model_override.lower() else "openai"
                )
            reasoning_key_env = os.getenv("LLM_REASONING_API_KEY_ENV")
            if not reasoning_key_env:
                reasoning_key_env = (
                    "ANTHROPIC_API_KEY" if reasoning_provider == "anthropic" else "OPENAI_API_KEY"
                )

            reasoning_config = ModelConfig(
                provider=reasoning_provider,
                model=reasoning_model_override,
                cost_tier=3,
                api_key_env=reasoning_key_env,
                base_url=os.getenv("LLM_REASONING_BASE_URL") or None,
            )
        elif os.getenv("NVIDIA_GLM_API_KEY") and os.getenv("NVIDIA_GLM_MODEL"):
            reasoning_config = ModelConfig(
                provider="openai",
                model=os.getenv("NVIDIA_GLM_MODEL", "z-ai/glm4.7"),
                cost_tier=3,
                api_key_env="NVIDIA_GLM_API_KEY",
                base_url=os.getenv("NVIDIA_BASE_URL") or None,
                max_tokens=8192,
            )
        elif os.getenv("OPENROUTER_API_KEY") and os.getenv("OPENROUTER_DS_R1_MODEL"):
            reasoning_config = ModelConfig(
                provider="openai",
                model=os.getenv("OPENROUTER_DS_R1_MODEL", "deepseek/deepseek-r1-0528:free"),
                cost_tier=2,
        ...
        code_model = os.getenv("LLM_CODE_MODEL", "gpt-4o")
        code_key_env = os.getenv("LLM_CODE_API_KEY_ENV", default_config.api_key_env)
        code_base_url = os.getenv("LLM_CODE_BASE_URL") or (
            default_config.base_url if default_config.provider == "openai" else None
        )
