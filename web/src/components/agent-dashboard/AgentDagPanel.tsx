...
function TaskDagNode({ data }: { data: { task: AgentTask } }) {
  const { task } = data
  const style = taskStatusToDagStyle(task.status)

  return (
    <div className={`rounded-lg border-2 px-3 py-2 min-w-[160px] max-w-[220px] ${style}`}>
...
export function AgentDagPanel() {
  ...
  const nodes: Node[] = useMemo(() => buildDagNodes(tasks), [tasks])
