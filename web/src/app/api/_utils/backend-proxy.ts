...
export async function proxyStream(
  ...
  try {
    const upstream = await fetchUpstream(req, upstreamUrl, method, options, {
      dispatcher: options.dispatcher ?? SSE_DISPATCHER,
    })
    const upstreamContentType = upstream.headers.get("content-type") || ""

    if (
      options.passthroughNonStreamResponse &&
    ...
    headers.set(
      "Content-Type",
      upstreamContentType || options.responseContentType || "text/event-stream",
    )
    headers.set("Cache-Control", "no-cache")
...
async function fetchUpstream(
  ...
  try {
    return await fetch(upstreamUrl, {
      ...init,
      method,
      headers: await resolveHeaders(req, method, options),
      body: await resolveBody(req, method),
      cache: options.cache,
