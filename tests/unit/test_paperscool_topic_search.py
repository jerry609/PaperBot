...
class _FakeSource:
    ...
    def search(self, *, query: str, branches, show_per_branch: int):
        if query == "icl compression":
            rows = [
                TopicSearchRecord(
                    source=self.name,
                    source_record_id="2510.00001",
                ...
                TopicSearchRecord(
                    source=self.name,
                    source_record_id="2025.acl-long.24@ACL",
                    title="UniICL: An Efficient ICL Framework Unifying Compression, Selection, and Generation",
            ...
            return [row for row in rows if row.source_branch in branches]

        if query == "kv cache acceleration":
            rows = [
                TopicSearchRecord(
                    source=self.name,
                    source_record_id="2412.19442",
...
class _FakeSourceFactory:
    @staticmethod
    def build_registry() -> TopicSearchSourceRegistry:
        registry = TopicSearchSourceRegistry()
        registry.register("fake_source", _FakeSource)
        return registry


def test_topic_search_normalizes_and_deduplicates_results():
    workflow = PapersCoolTopicSearchWorkflow(source_registry=_FakeSourceFactory.build_registry())

    result = workflow.run(
        queries=["ICL压缩", " kv cache加速 ", "ICL 压缩"],
        branches=["arxiv", "venue"],
        sources=["fake_source", "fake_source"],
        top_k_per_query=3,
    )
    ...
    assert uniicl_item["sources"] == ["fake_source"]
    ...
    assert result["summary"]["source_breakdown"] == {"fake_source": 2}
